...
func draw_grid(length:int) -> void:
	...
	for i in range(-length, length):
		#row
		var multiplied:int = i * Constants.SPATIAL_HASH_SIZE
		# y/Rows. Lines are horizontal, measures vertical
		draw_dashed_line(
			Vector2(-Constants.SPATIAL_HASH_SIZE * length, multiplied),
			Vector2(Constants.SPATIAL_HASH_SIZE * length, multiplied),
			Color.GOLD,
			-1.0,
			12.0
		...
		draw_dashed_line(
			Vector2(multiplied, -Constants.SPATIAL_HASH_SIZE * length),
			Vector2(multiplied, Constants.SPATIAL_HASH_SIZE * length),
			Color.LIGHT_BLUE,
			-1.0,
			12.0
