...
@export_range(-0.5,0.5,0.02) var x_bias:float = 0.25
...
func _tick(_delta: float) -> Status:
	...
	var new_pos:Vector2 = blackboard.get_var(target_variable,agent,false).global_position + (Vector2.from_angle(TAU*randf()) * bias) * randf_range(min_distance,max_distance)
